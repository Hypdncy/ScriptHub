...
# 设置保存文件
ipaddress=$(ip address | grep -o -P '(?<=inet )\d+\.\d+\.\d+\.\d+(?=\/2)' | head -n 1)
filename=$ipaddress'_'$(hostname)'_'$(whoami)'_'$(date +%s)_log'.log'
vuln="${ipaddress}_$(hostname)_$(whoami)_$(date +%s)_vuln.log"
...
check_base() {
    ...
    #服务器SN
    echo -e "服务器SN: \t$(dmidecode -t1 | grep -o -P '(?<=Serial Number: ).*')" | tee -a "$filename"
    #uptime
    echo -e "Uptime: \t$(uptime | awk -F ',' '{print $1}')" | tee -a "$filename"
    #系统负载
    echo -e "系统负载: \t$(uptime | awk '{print $9" "$10" "$11" "$12" "$13}')" | tee -a "$filename"
    #cpu信息
    echo -e "CPU info:\t$(grep -o -P '(?<=model name\t: ).*' </proc/cpuinfo | head -n 1)" | tee -a "$filename"
    #cpu核心
    echo -e "CPU 核心:\t$(cat /proc/cpuinfo | grep 'processor' | sort | uniq | wc -l)" | tee -a "$filename"
    #ipaddress
    ipaddress=$(ifconfig | grep -o -P '(?<=inet |inet addr:)\d+\.\d+\.\d+\.\d+' | grep -v -P '127.0.0.1') >/dev/null 2>&1
    ...
    grep -v -P "#" </etc/fstab | awk '{print $1,$2,$3}' | tee -a "$filename"
    echo -e "\n" | tee -a "$filename"
    #安装软件
    ...
    for prog in "${cmdline[@]}"; do
        ...
        if [ "$soft" ] 2>/dev/null; then
            echo -e "$soft" | grep -o -P '\w+$' | tee -a "$filename"
        fi
    done
    echo -e "\n" | tee -a "$filename"
    #HOSTS
    echo -e "\e[00;31m[+]/etc/hosts \e[00m" | tee -a "$filename"
    cat /etc/hosts | grep -v -P "#" | tee -a "$filename"
    echo -e "\n" | tee -a "$filename"
}
...
check_network() {
    ...
    netstat -tulpen | grep -P 'tcp|udp.*' | tee -a "$filename"
    echo -e "\n" | tee -a "$filename"
    #对外开放端口
    echo -e "\e[00;31m[+]对外开放端口\e[00m" | tee -a "$filename"
    netstat -tulpen | awk '{print $1,$4}' | grep -o -P '.*0.0.0.0:(\d+)|:::\d+' | tee -a "$filename"
    echo -e "\n" | tee -a "$filename"
    #网络连接
    echo -e "\e[00;31m[+]网络连接\e[00m" | tee -a "$filename"
    netstat -antop | grep -P ESTAB | tee -a "$filename"
    echo -e "\n" | tee -a "$filename"
    #连接状态
    ...
    grep -o -P '\d+\.\d+\.\d+\.\d+' </etc/resolv.conf | tee -a "$filename"
    echo -e "\n" | tee -a "$filename"
    #ARP
...
check_crontab() {
    ...
    crontab -u root -l | grep -v -P '#' | tee -a "$filename"
    ls -alht /etc/cron.*/* | tee -a "$filename"
    echo -e "\n" | tee -a "$filename"

    #crontab可疑命令
    echo -e "\e[00;31m[+]Crontab Backdoor \e[00m" | tee -a "$vuln"
    grep -P '((?:useradd|groupadd|chattr)|(?:wget\s|curl\s|tftp\s\-i|scp\s|sftp\s)|(?:bash\s\-i|fsockopen|nc\s\-e|sh\s\-i|\"/bin/sh\"|\"/bin/bash\"))' /etc/cron* /var/spool/cron/* | tee -a "$vuln"
    echo -e "\n" | tee -a "$vuln"

    echo -e "\e[00;31m[+]Auto Init \e[00m" | tee -a "$vuln"
    ls -alr /etc/init.d | tee -a "$vuln"
    ls -alr /etc/rc* | tee -a "$vuln"
    ls -alr /etc/profile.d | tee -a "$vuln"
    echo -e "\n" | tee -a "$vuln"
...
check_user() {
    ...
    cat /etc/passwd | grep -v -P 'nologin$|false$' | tee -a "$filename"
    echo -e "\n" | tee -a "$filename"
    echo -e "\e[00;31m[+]passwd文件修改日期: \e[00m$(stat /etc/passwd | grep -o -P '(?<=Modify: ).*')" | tee -a "$filename"
    echo -e "\n" | tee -a "$filename"
    echo -e "\e[00;31m[+]sudoers(请注意NOPASSWD)\e[00m" | tee -a "$filename"
    cat /etc/sudoers | grep -v -P '#' | sed -e '/^$/d' | grep -P ALL | tee -a "$filename"
    echo -e "\n" | tee -a "$filename"
    ...
    echo -e "\n" | tee -a "$filename"
    echo "登陆ip: $(grep -P accepted /var/log/secure /var/log/auth.* 2>/dev/null | grep -o -P '\d+\.\d+\.\d+\.\d+' | sort | uniq)" | tee -a "$filename"
    echo -e "\n" | tee -a "$filename"
}
...
check_service() {
    ...
    ls -alht /etc/systemd/system/*.service | grep -v -P 'dbus-org' | tee -a "$filename"
    echo -e "\n" | tee -a "$filename"

    echo -e "\e[00;31m[+]列出所有自启动服务 \e[00m" | tee -a "$filename"
...
check_file() {
    ...
    for soft in "${cmdline[@]}"; do
        echo -e "文件：$soft\t\t\t修改日期：$(stat $soft | grep -o -P '(?<=Modify: )[\d-\s:]+')" | tee -a "$vuln"
    done
    ...
    find /etc /bin /lib /sbin /dev /root/ /home /tmp /var /usr ! -path "/var/log*" ! -path "/var/spool/exim4*" ! -path "/var/backups*" -mtime -7 -type f | grep -v -P '\.log|cache|vim|/share/|/lib/|.zsh|.gem|\.git|LICENSE|README|/_\w+\.\w+|\blogs\b|elasticsearch|nohup|i18n' | xargs -i{} ls -alh {} | tee -a "$filename"
    echo -e "\n" | tee -a "$filename"
    #近7天改动
    echo -e "\e[00;31m[+]近七天文件改动 ctime \e[00m" | tee -a "$filename"
    find /etc /bin /lib /sbin /dev /root/ /home /tmp /var /usr ! -path "/var/log*" ! -path "/var/spool/exim4*" ! -path "/var/backups*" -ctime -7 -type f | grep -v -P '\.log|cache|vim|/share/|/lib/|.zsh|.gem|\.git|LICENSE|README|/_\w+\.\w+|\blogs\b|elasticsearch|nohup|i18n' | xargs -i{} ls -alh {} | tee -a "$filename"
    echo -e "\n" | tee -a "$filename"
    #大文件100mb
    #有些黑客会将数据库、网站打包成一个文件然后下载
    echo -e "\e[00;31m[+]大文件>200mb \e[00m" | tee -a "$filename"
    find / ! -path "/proc/*" ! -path "/sys/*" ! -path "/run/*" ! -path "/boot/*" -size +200M -exec ls -alht {} + 2>/dev/null | grep -P '\.gif|\.jpeg|\.jpg|\.png|\.zip|\.tar.gz|\.tgz|\.7z|\.log|\.xz|\.rar|\.bak|\.old|\.sql|\.1|\.txt|\.tar|\.db|/\w+$' | grep -v -P 'ib_logfile|ibd|mysql-bin|mysql-slow|ibdata1' | tee -a "$filename"
    echo -e "\n" | tee -a "$filename"
    #敏感文件
    echo -e "\e[00;31m[+]敏感文件 \e[00m" | tee -a "$vuln"
    find / ! -path "/lib/modules*" ! -path "/usr/src*" ! -path "/snap*" ! -path "/usr/include/*" -regextype posix-extended -regex '.*sqlmap|.*msfconsole|.*\bncat|.*\bnmap|.*nikto|.*ettercap|.*tunnel\.(php|jsp|asp|py)|.*/nc\b|.*socks.(php|jsp|asp|py)|.*proxy.(php|jsp|asp|py)|.*brook.*|.*frps|.*frpc|.*aircrack|.*hydra|.*miner|.*/ew$' -type f | grep -v -P '/lib/python' | xargs -i{} ls -alh {} | tee -a "$vuln"
    echo -e "\n" | tee -a "$vuln"

    echo -e "\e[00;31m[+]可疑黑客文件 \e[00m" | tee -a "$vuln"
    find /root /home /opt /tmp /var/ /dev -regextype posix-extended -regex '.*wget|.*curl|.*openssl|.*mysql' -type f 2>/dev/null | xargs -i{} ls -alh {} | grep -v -P '/pkgs/|/envs/' | tee -a "$vuln"
    echo -e "\n" | tee -a "$vuln"
}
...
check_ssh() {
    ...
    # 自动扫描两种情况
    grep -P 'authentication failure' /var/log/secure* /var/log/auth.* | awk '{print $14}' | awk -F '=' '{print $2}' | grep -P '\d+\.\d+\.\d+\.\d+' | sort | uniq -c | sort -nr | head -n 25 | tee -a "$filename"
    echo -e "\n" | tee -a "$filename"
    #SSHD
    echo -e "\e[00;31m[+]SSHD \e[00m" | tee -a "$filename"
    echo -e "/usr/sbin/sshd"
    stat /usr/sbin/sshd | grep -P 'Access|Modify|Change' | tee -a "$filename"
    echo -e "\n" | tee -a "$filename"
...
check_webshell() {
    ...
    grep -R --include '*.php' -l -s -i -P 'array_map\(|pcntl_exec\(|proc_open\(|popen\(|assert\(|phpspy|c99sh|milw0rm|eval?\(|\(gunerpress|\(base64_decoolcode|spider_bc|shell_exec\(|passthru\(|base64_decode\s?\(|gzuncompress\s?\(|gzinflate|\(\$\$\w+|call_user_func\(|call_user_func_array\(|preg_replace_callback\(|preg_replace\(|register_shutdown_function\(|register_tick_function\(|mb_ereg_replace_callback\(|filter_var\(|ob_start\(|usort\(|uksort\(|uasort\(|GzinFlate\s?\(|\$\w+\(\d+\)\.\$\w+\(\d+\)\.|\$\w+=str_replace\(|eval\/\*.*\*\/\(' $webpath | tee -a "$vuln"
    grep -R --include '*.php' -l -s -i -P '^(\xff\xd8|\x89\x50|GIF89a|GIF87a|BM|\x00\x00\x01\x00\x01)[\s\S]*<\?\s*php' $webpath | tee -a "$vuln"
    grep -R --include '*.php' -l -s -i -P '\b(assert|eval|system|exec|shell_exec|passthru|popen|proc_open|pcntl_exec)\b[\/*\s]*\(+[\/*\s]*((\$_(GET|POST|REQUEST|COOKIE)\[.{0,25})|(base64_decode|gzinflate|gzuncompress|gzdecode|str_rot13)[\s\(]*(\$_(GET|POST|REQUEST|COOKIE)\[.{0,25}))' $webpath | tee -a "$vuln"
    grep -R --include '*.php' -l -s -i -P '\$\s*(\w+)\s*=[\s\(\{]*(\$_(GET|POST|REQUEST|COOKIE)\[.{0,25});[\s\S]{0,200}\b(assert|eval|system|exec|shell_exec|passthru|popen|proc_open|pcntl_exec)\b[\/*\s]*\(+[\s"\/*]*(\$\s*\1|((base64_decode|gzinflate|gzuncompress|gzdecode|str_rot13)[\s\("]*\$\s*\1))' $webpath | tee -a "$vuln"
    grep -R --include '*.php' -l -s -i -P '\b(filter_var|filter_var_array)\b\s*\(.*FILTER_CALLBACK[^;]*((\$_(GET|POST|REQUEST|COOKIE|SERVER)\[.{0,25})|(eval|assert|ass\\x65rt|system|exec|shell_exec|passthru|popen|proc_open|pcntl_exec))' $webpath | tee -a "$vuln"
    grep -R --include '*.php' -l -s -i -P "\b(assert|eval|system|exec|shell_exec|passthru|popen|proc_open|pcntl_exec|include)\b\s*\(\s*(file_get_contents\s*\(\s*)?[\'\"]php:\/\/input" $webpath | tee -a "$vuln"
    echo -e "\n" | tee -a "$vuln"
    #JSP webshell查杀
    echo -e "\e[00;31m[+]JSP webshell查杀\e[00m" | tee -a "$vuln"
    grep -R --include '*.jsp' -l -s -i -P '<%@\spage\simport=[\s\S]*\\u00\d+\\u00\d+|<%@\spage\simport=[\s\S]*Runtime.getRuntime\(\).exec\(request.getParameter\(|Runtime.getRuntime\(\)' $webpath | tee -a "$vuln"
    echo -e "\n" | tee -a "$vuln"
}
...
check_poison() {
    ...
    pip freeze | grep -P "istrib|djanga|easyinstall|junkeldat|libpeshka|mumpy|mybiubiubiu|nmap-python|openvc|python-ftp|pythonkafka|python-mongo|python-mysql|python-mysqldb|python-openssl|python-sqlite|virtualnv|mateplotlib|request=" | tee -a "$vuln"
    echo -e "\n" | tee -a "$vuln"
    echo -e "\e[00;31m[+]Python3 pip 检测\e[00m" | tee -a "$vuln"
    pip3 freeze | grep -P "istrib|djanga|easyinstall|junkeldat|libpeshka|mumpy|mybiubiubiu|nmap-python|openvc|python-ftp|pythonkafka|python-mongo|python-mysql|python-mysqldb|python-openssl|python-sqlite|virtualnv|mateplotlib|request=" | tee -a "$vuln"
    echo -e "\n" | tee -a "$vuln"
}
...
check_miner() {
    ...
    ps aux | grep -P "work32|work64|/tmp/secure.sh|/tmp/auth.sh" | grep -v -P 'grep'
...
check_redis() {
    ...
    cat /etc/redis/redis.conf 2>/dev/null | grep -P '(?<=requirepass )(test|123456|admin|root|12345678|111111|p@ssw0rd|test|qwerty|zxcvbnm|123123|12344321|123qwe|password|1qaz|000000|666666|888888)' | tee -a "$vuln"
    echo -e "\n" | tee -a "$vuln"
}
check_nvidia() {
    echo -e "############ Nvidia/挖矿检查 ############\n" | tee -a "$vuln"
    echo -e "\e[00;31m[+]Nvidia驱动篡改检测\e[00m" | tee -a "$vuln"
    ll /usr/bin/nvidia* | tee -a "$vuln"
}
...
check_process() {
    ...
    str_procs="${procs[*]}"
    reg_procs="${str_procs/ /|}"
    ps -ef | grep -v grep | grep -P "${reg_procs}" | awk '{print $1}'
}
